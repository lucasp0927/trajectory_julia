...
#    init_parallel(paras.phy_para,paras.sim_para,paras.time_arr,paras.freq_arr,paras.t_span,traj_selected,paras.probe)
    ...
    @time for i in 1:iter
        ...
        @sync @distributed for idx in CartesianIndices(output_matrix[1,1,:,:,1])
            fidx = idx[1]
            tidx = idx[2]
            output_matrix[:,:,fidx,tidx,i],output[fidx,tidx,i] = transmission2d(time_range[tidx],freq_range[fidx],atom_arr,d)
        end
    end
...
function transmission2d(t::Float64,detune::Float64,atom_arr::Array{Int64,2},d)
