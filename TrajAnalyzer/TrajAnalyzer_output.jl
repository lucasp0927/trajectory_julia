function output_movie_traj_2d(config,filename)
    mov_tspan = collect(config["tstart"]:config["tdiv"]:config["tend"])
    mov_range = [promote(config["range"]...)...]
    mov_res = config["res"]
    output_movie_2d(mov_tspan,mov_range,mov_res,filename,traj=true)
end

function output_movie_traj_3d(config,filename)
    mov_tspan = collect(config["tstart"]:config["tdiv"]:config["tend"])
    mov_range = [promote(config["range"]...)...]
    mov_res = config["res"]
    output_movie_3d(mov_tspan,mov_range,mov_res,filename)
end

function output_movie_traj_flux_2d(config,filename,result,tspan,flux)
    flux_arr = reduce((x,y)->cat(2,x,y),values(flux))
    #TODO: sort by time
    mov_tspan = collect(config["tstart"]:config["tdiv"]:config["tend"])
    mov_range = [promote(config["range"]...)...]
    mov_res = config["res"]
    output_movie_2d(mov_tspan,mov_range,mov_res,filename,traj=true,result=result,tspan=tspan)
end

function output_movie_data_2d(config,filename)
    @assert occursin(".mat",filename)
    mov_tspan = collect(config["tstart"]:config["tdiv"]:config["tend"])
    mov_range = [promote(config["range"]...)...]
    potential = cat(pmap(t->Fields.composite(mov_range,t),mov_tspan)...,dims=3)
...
function output_movie_3d(mov_tspan,range,res,filename)
    @assert occursin(".mp4",filename)
    #pre render potential to find v_min and v_max
    # output_pre = pmap(t->Fields.composite(range,t),mov_tspan)
    # v_min = minimum(map(minimum,output_pre))
...
function output_image_gp_2d(t,range,filename;v_min=0.0,v_max=0.0,xres=1300,yres=1000,save_data=false,data_filename = "")
    ...
    cp(image_folder*"/data.png",filename,force=true)
    if save_data
        cp(image_folder*"/data.h5",data_filename,force=true)
    end
    rm(image_folder,recursive=true)
end

function output_image_gp_traj_2d(t,range,res_x,res_y,filename;v_min=0.0,v_max=0.1,tdiv=0.0)
    if (v_max-v_min <= eps())
        v_max = v_max + 0.1
    end
    tmp = (Trajs[t,:])[1:2,:]
