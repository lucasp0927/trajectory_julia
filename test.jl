...
function main()
    ...
    println(Fields.typeoffield(vfn))
    ...
    sf4 = Fields.func2field(ScalarField{Complex{Float64},3},(x,y,z)->x+0im,(2,2,2),(2,2,2),(4,12,2))
    #########
    sfn = Fields.ScalarFieldNode{3}([sf3,sf4])
    sfn2 = Fields.ScalarFieldNode{3}([sf1,sf2])
    println("composite test!")
    println(Fields.typeoffield(sfn))
    Fields.composite(sfn,0.0)
    println(Fields.geometry(sfn))
    println(Fields.geometry(sfn2))
