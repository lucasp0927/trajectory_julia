...
@inbounds function sample{T<:ComplexOrFloat}(f::ScalarField{T,2},pos::Vector{Float64},t::Real;order::Integer = 3)
    fpos::Vector{Float64} = collect(f.position)
    fres::Vector{Float64} = collect(f.res)
    fsize::Vector{Float64} = collect(f.size)
    rel_pos::Vector{Float64} = pos-fpos
    pidx::Vector{Int64} = round(Int64,div(rel_pos,fres)+1)
...
function sample_inner{T<:ComplexOrFloat}(::Type{T},f::ScalarFieldNode{2},pos::Vector{Float64},t::Real;order::Integer = 3)
    ...
    output::Array{Float64,2} = zeros(Float64,(4,4))#TODO: Float64 should be T
    vf_output::Array{T,3} = zeros(T,(3,4,4))
    if ~isempty(sf_arr)
    ...
    output .+= vf_output_abs2::Array{Float64,2}
    return (output*f.scaling(t)::ComplexOrFloat)::Array{Float64,2}
end
...
@inbounds @fastmath function in_field{T<:Field}(f::T,pos::Vector{Float64})
    ...
    return all(i->f.res[i]::Float64<(pos[i]-f.position[i]::Float64)<(f.size[i]::Float64-f.res[i]::Float64),1:length(pos))::Bool
end
...
#    A = float(real(A)) #TODO: change this
    res = f.res::Tuple{Vararg{Float64}}
...
function itp_bicubic_grad{T<:ComplexOrFloat}(A::Array{T},pos::Tuple,res::Tuple)
    arr = Array(T,4)
