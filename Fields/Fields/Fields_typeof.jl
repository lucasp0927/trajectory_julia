...
function typeoffield(f::VectorFieldNode)
    ...
    typeintersect(reduce((x,y)-> begin
    Union{x,y},
     f_member_type),ComplexOrFloat)==Float64 ? Float64 : Complex{Float64}
end
...
function test_typeof()
    for dim in [2,3]
        float_sf1 = zero_field(ScalarField{Float64,dim},repmat([100],dim),repmat([0.0],dim),repmat([100.0],dim))
        float_sf2 = zero_field(ScalarField{Float64,dim},repmat([100],dim),repmat([0.0],dim),repmat([100.0],dim))
        complex_sf = zero_field(ScalarField{Complex{Float64},dim},repmat([100],dim),repmat([0.0],dim),repmat([100.0],dim))
        ...
        float_vf1 = zero_field(VectorField{Float64,dim},repmat([100],dim),repmat([0.0],dim),repmat([100.0],dim))
        float_vf2 = zero_field(VectorField{Float64,dim},repmat([100],dim),repmat([0.0],dim),repmat([100.0],dim))
        complex_vf = zero_field(VectorField{Complex{Float64},dim},repmat([100],dim),repmat([0.0],dim),repmat([100.0],dim))
