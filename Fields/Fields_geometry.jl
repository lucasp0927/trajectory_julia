...
function test_geometry()
    ...
    =#
    for dim in [2,3]
        float_sf1 = zero_field(ScalarField{Float64,dim},repeat([201],dim), repeat([50.0],dim),repeat([50.0],dim),name = "float_sf1")
        @test geometry(float_sf1) == Dict("res"=>repeat([0.25],dim),"size"=>repeat([50.0],dim),"pos"=>repeat([50.0],dim))
        float_sf2 = zero_field(ScalarField{Float64,dim},repeat([101],dim),repeat([0.0],dim),repeat([100.0],dim),name = "float_sf2")
        @test geometry(float_sf2) == Dict("res"=>repeat([1.0],dim),"size"=>repeat([100.0],dim),"pos"=>repeat([0.0],dim))
        complex_sf = zero_field(ScalarField{Complex{Float64},dim},repeat([121],dim),repeat([20.0],dim),repeat([60.0],dim),name = "complex_sf")
        ...
        @test geometry(sfn1) == Dict("res"=>repeat([0.5],dim),"size"=>repeat([80.0],dim),"pos"=>repeat([20.0],dim))
        sfn2 = ScalarFieldNode{dim}([float_sf2,sfn1],name = ascii("sfn2"))
        @test geometry(sfn2) == Dict("res"=>repeat([1.0],dim),"size"=>repeat([100.0],dim),"pos"=>repeat([0.0],dim))
        set_geometry!(sfn2)
        @test sfn1.res == repeat([0.5],dim)
        @test sfn1.size ==repeat([80.0],dim)
        @test sfn1.position == repeat([20.0],dim)
        @test sfn2.res == repeat([1.0],dim)
        @test sfn2.size == repeat([100.0],dim)
        @test sfn2.position == repeat([0.0],dim)
    end
    for dim in [2,3]
        float_vf1 = zero_field(VectorField{Float64,dim},repeat([201],dim), repeat([50.0],dim),repeat([50.0],dim),name = "float_vf1")
        @test geometry(float_vf1) == Dict("res"=>repeat([0.25],dim),"size"=>repeat([50.0],dim),"pos"=>repeat([50.0],dim))
        float_vf2 = zero_field(VectorField{Float64,dim},repeat([101],dim),repeat([0.0],dim),repeat([100.0],dim),name = "float_vf2")
        @test geometry(float_vf2) == Dict("res"=>repeat([1.0],dim),"size"=>repeat([100.0],dim),"pos"=>repeat([0.0],dim))
        complex_vf = zero_field(VectorField{Complex{Float64},dim},repeat([121],dim),repeat([20.0],dim),repeat([60.0],dim),name = "complex_vf")
        ...
        @test geometry(vfn1) == Dict("res"=>repeat([0.5],dim),"size"=>repeat([80.0],dim),"pos"=>repeat([20.0],dim))
        vfn2 = VectorFieldNode{dim}([float_vf2,vfn1],name = ascii("vfn2"))
        @test geometry(vfn2) == Dict("res"=>repeat([1.0],dim),"size"=>repeat([100.0],dim),"pos"=>repeat([0.0],dim))
        set_geometry!(vfn2)
        @test vfn1.res == repeat([0.5],dim)
        @test vfn1.size ==repeat([80.0],dim)
        @test vfn1.position == repeat([20.0],dim)
        @test vfn2.res == repeat([1.0],dim)
